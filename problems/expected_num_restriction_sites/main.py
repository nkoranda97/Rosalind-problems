...
class Solution:
    ...
    def calculate_expected_num_restriction_sites(self) -> List[float]:
        ...
        for gc_percent in self.array:
            ...
            solution.append(round(reduce(mul, [frequencies[nuc] for nuc in self.s]) * (mult_factor), 3))
        
        return solution
    
def main() -> None:
    solution = Solution('problems/expected_num_restriction_sites/rosalind_eval.txt')
    expected_num_restriction_sites: List[float] = solution.calculate_expected_num_restriction_sites()
    printable_answer: str = ' '.join([str(num) for num in expected_num_restriction_sites])
