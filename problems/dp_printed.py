...
def longest_subsequence(numList, comparison_func, n):
    ...
    for i in range(1, n):
        for j in range(i):
            if (
                comparison_func(numList[i], numList[j])
                and len(nums[i]) < len(nums[j]) + 1
            ):
                nums[i] = nums[j] + [numList[i]]
                print(
                    f"Updated nums[{i}] to be {nums[i]} because numList[{i}] ({numList[i]}) {'>' if comparison_func == gt else '<'} numList[{j}] ({numList[j]})"
                )
